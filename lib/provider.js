...
export default class ReferenceProvider {
  ...
  buildWordList(references) {
    ...
    for (let citation of references) {
      if (citation.title && (citation.authors || citation.editors)) {
        var author;
        citation.title = citation.title.replace(/(^\{|\}$)/g, "");
        citation.prettyTitle =
            this.prettifyTitle(citation.title);

        citation.fuzzyLabel = citation.title;

        if (citation.authors) {
          for (author of Array.from(citation.authors)) {
            citation.fuzzyLabel += ` ${author.given} ${author.family}`;
          }
        }

        if (citation.editors) {
          for (let editor of citation.editors) {
            citation.fuzzyLabel += ` ${editor.given} ${editor.family}`;
          }
        }

        if (citation.authors) {
          citation.prettyAuthors =
              this.prettifyAuthors(citation.authors);
          ...
          for (author of Array.from(citation.authors)) {
            possibleWords.push({
              ...
              type: citation.type,
              in: citation.in || citation.journal || citation.booktitle,
              url: (citation.url !== null) ? citation.url : undefined
            });
          }
        } else {
          possibleWords.push({
