...
const fs = require("fs");
const bibtexParse = require("zotero-bibtex-parse");
...
(ReferenceProvider = (function() {
  ReferenceProvider = class ReferenceProvider {
    static initClass() {
      atom.deserializers.add(this);
      this.version = 4;

      // TODO Could add a system to register additional readers
      this.prototype.fileReaders = [new BibtexReader(), new CiteprocReader(), new YamlReader()];
    }
      ...
      for (let citation of references) {
        if (citation.entryTags && citation.entryTags.title && (citation.entryTags.authors || citation.entryTags.editors)) {
          ...
          if (citation.entryTags.editors !== null) {
            for (let editor of Array.from(citation.entryTags.editors)) {
    ...
    */
    ...
    prettifyName(person, inverted, separator) {
      ...
      return ((person.personalName != null) ? person.personalName : '') +
        ((person.personalName != null) && (person.familyName != null) ? separator : '') +
        ((person.familyName != null) ? person.familyName : '');
    }
  };
  ReferenceProvider.initClass();
