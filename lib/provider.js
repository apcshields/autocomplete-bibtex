...
export default class ReferenceProvider {
  ...
  getSuggestions({editor, bufferPosition}) {
    ...
    return new Promise(resolve => {
      if (prefix[0] === "@") {
        ...
        const hits = fuzzaldrin.filter(this.possibleWords, normalizedPrefix, {key: 'fuzzyLabel'});
        ...
        for (let word of hits) {
          ...
          suggestions.push({
            text: this.resultTemplate.replace("[key]", word.id),
            displayText: word.label,
            replacementPrefix: prefix,
            leftLabel: word.id,
            rightLabel: word.author,
            className: word.type,
  ...
  buildWordList(references) {
    ...
    for (let reference of references) {
      if (reference.title && (reference.authors || reference.editors)) {
        ...
        if (reference.authors) {
          for (let author of Array.from(reference.authors)) {
        ...
        let prettyAuthors = '';
        if (reference.authors) {
          prettyAuthors = this.prettifyAuthors(reference.authors);
        }
        reference.prettyAuthors = prettyAuthors;

        possibleWords.push({
          id: reference.id,
          label: reference.prettyTitle,
          author: prettyAuthors,
          type: reference.type,
          in: reference.in || reference.journal || reference.booktitle,
          url: (reference.url) ? reference.url : undefined
        });
      }
    }
