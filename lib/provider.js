    ...
    return new Promise(resolve => {
      if (prefix[0] === "@") {
        ...
        for (let hit of hits) {
          ...
          suggestions.push({
            text: this.resultTemplate.replace("[key]", hit.id),
            displayText: hit.prettyTitle,
            replacementPrefix: prefix,
            leftLabel: hit.id,
            rightLabel: hit.prettyAuthors,
            className: hit.type,
            iconHTML: '<i class="icon-mortar-board"></i>',
            description: (hit.in !== null) ? hit.in : undefined,
            descriptionMoreURL: (hit.url !== null) ? hit.url : undefined
          });
        }
