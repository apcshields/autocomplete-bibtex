...
export default class ReferenceProvider {
  ...
  buildWordList(references) {
    ...
    for (let citation of references) {
      if (citation.tags && citation.tags.title && (citation.tags.authors || citation.tags.editors)) {
        var author;
        citation.tags.title = citation.tags.title.replace(/(^\{|\}$)/g, "");
        citation.tags.prettyTitle =
            this.prettifyTitle(citation.tags.title);

        citation.fuzzyLabel = citation.tags.title;

        if (citation.tags.authors) {
          for (author of Array.from(citation.tags.authors)) {
            citation.fuzzyLabel += ` ${author.given} ${author.family}`;
          }
        }

        if (citation.tags.editors) {
          for (let editor of citation.tags.editors) {
            citation.fuzzyLabel += ` ${editor.given} ${editor.family}`;
          }
        }

        if (citation.tags.authors) {
          citation.tags.prettyAuthors =
              this.prettifyAuthors(citation.tags.authors);
          ...
          for (author of Array.from(citation.tags.authors)) {
            possibleWords.push({
              ...
              type: citation.tags.type,
              in: citation.tags.in || citation.tags.journal || citation.tags.booktitle,
              url: (citation.tags.url !== null) ? citation.tags.url : undefined
            });
          }
        } else {
          possibleWords.push({
  ...
  updateReferences(referenceFiles) {
    ...
    this.possibleWords = this.buildWordList(this.references);
  }
