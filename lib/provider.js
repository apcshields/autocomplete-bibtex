...
export default class ReferenceProvider {
  ...
  update() {
    ...
    this.registerReferenceFiles(this.referenceFiles)
    this.references = Promise.all(this.watchedFiles.map(f => f.references))
  }
  ...
  //         persistent: true,
  //         // TODO not sure await write finish is working...
  //         // might be just as safe to use plain fs watch and manually poll.
  ...
  readReferenceFile(file) {
    const fileType = file.split('.').pop()
    const reader = getFileReader(fileType)
