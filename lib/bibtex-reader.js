...
export default class BibtexReader {
  ...
  parseBibtexAuthors(citations) {
    ...
    for (let citation of citations) {
      if (citation.tags) {
        if (citation.tags.author) {
          citation.tags.authors = this.cleanAuthors(citation.tags.author.split(' and '));
        }

        if (citation.tags.editor) {
          citation.tags.editors = this.cleanAuthors(citation.tags.editor.split(' and '));
        }
