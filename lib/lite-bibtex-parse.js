...
class BibtexParser {
  ...
  key_equals_value() {
    ...
    if (this.tryMatch("=")) {
      ...
      const convertFields = ['author', 'title'];
  ...
  key_value_list() {
    ...
    this.currentEntry.tags = {};
    this.currentEntry.tags[kv[0]] = kv[1];
    while (this.tryMatch(",")) {
      this.match(",");
  ...
  alernativeid() {
    this.entries.forEach(function(entry) {
      if (!entry.id && entry.tags) {
        entry.id = '';
        if (entry.tags.author) {
          entry.id += entry.tags.author.split(',')[0] += ', ';
        }
        entry.id += entry.tags.year;
      }
    });
  }
